...
Public Class cDelivery
	...
	Property ShippedQty As Double
	Property RemainQty As Double
	...
	Private Function AddDelivery(cmd As SqlCommand) As Boolean
		Try
			...
			vals = vals & ",0" '& AddString(dtr("Shipped_Quantity"), DbType.Double, "NULL")
			vals = vals & "," & AddString(Quantity, DbType.Double, "NULL")
	...
	Private Function LoadClass(SODetail As Integer, cmd As SqlCommand) As Boolean
		cmd.CommandText = "SELECT * FROM Delivery WHERE SO_Detail=" & SODetail & " AND Invoice IS NULL AND Invoice_Date IS NULL" _
			& " AND Shipped_Date IS NOT NULL AND Packlist IS NULL"
		Using dt As New DataTable
			...
			Else
				...
				Me.SODetail = SODetail
				Quantity = dtr("Quantity")
				ShippedDate = dtr("Shipped_Date")
				PromisedDate = dtr("Promised_Date")
				DeliveryOID = dtr("ObjectID").ToString
				ShippedQty = dtr("Shipped_Quantity")
				RemainQty = dtr("Remaining_Quantity")
				Return True
			End If
		End Using
	End Function

	Function Load(SODetail As Integer)
		Using con As New SqlConnection(JBConnection)
			con.Open()
			Using cmd As New SqlCommand
				cmd.Connection = con
		...
		End Using
	End Function
	Function Load(SODetail As Integer, cmd As SqlCommand)
		Return LoadClass(SODetail, cmd)
	End Function
