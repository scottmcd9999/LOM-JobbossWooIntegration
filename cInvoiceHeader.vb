...
Public Class cInvoiceHeader
	...
	Function AddInvoiceHeader() As Boolean
		Try
			...
			vals = vals & "," & AddString(plh.Customer)
			...
			vals = vals & "," & AddString(cust.TaxCode,, "NULL")
			vals = vals & "," & AddString(DocumentDate, DbType.Date)
			vals = vals & ",NULL"
			vals = vals & "," & AddString(DocumentDate, DbType.Date)
			vals = vals & ",0,0,0,'N','INV','Un-Posted'"
			...
			If finpref.BaseCurrency <> cust.CurrencyDef Then
				...
				CurrencyConvRate = curdef.ConversionRate(finpref.BaseCurrency, cust.CurrDef)
			Else
				CurrencyConvRate = 1
			End If
			vals = vals & "," & CurrencyConvRate
			vals = vals & "," & cust.CurrDef
			vals = vals & ",1"
			...
			vals = vals & "," & cust.RemitAddr.Address
